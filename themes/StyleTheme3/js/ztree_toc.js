			...
			timeout = setTimeout(function() {
				...
				for (var i = 0, c = opts._header_offsets.length; i < c; i++) {
					...
					if (opts._header_offsets[i] >= (top + 5) ) {
                        ...
                        //opts._header_offsets[14] = 5670.5
                        console.log("zukgit-height:"+$(window).height());
						console.log("scrollTop-height:"+$('#tree').scrollTop());
						var headTop = (i-1) * 18 + 5;
						console.log("headTop:"+headTop);
						if(headTop > $(window).height()){ // 如果高度大于 满屏
							var value = headTop/$(window).height();
							var number = Math.floor(value);
							$('#tree').animate({scrollTop: number * $(window).height() }, 50);
							
						}
						if(headTop <  $(window).height() ){
							$('#tree').animate({scrollTop: 0 }, 50);
						}
						break;
					}
				}
